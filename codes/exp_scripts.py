...
def exp5_rungzip_specific_0707():
    ...
    exp_name = "exp5_rungzip_specificacc_0707"
...
def exp3_rungzip_benchmark_0706():
    exp_name = "exp3_rungzip_benchmark_0708_onebyone"
...
def exp8_rungzip_downsample_0708():
    
    exp_name = "exp8_rungzip_downsample_0708"
    ...
    combinations = itertools.product(seeds, combinations)
    # flatten to (seed, dataset, n_shot, decimal, k)
    ...
    with tqdm(total=len(combinations)) as pbar:
        with ThreadPoolExecutor(max_workers=1) as executor:
            for seed, dataset, n_shot, decimal, k in tqdm(combinations):
                ...
                args.benchmark = True
                fut = executor.submit(
                    lambda x: subprocess.run(["python", "main.py", f"--scheme=gzip", f"--exp_name={x.exp_name}", f"--dataset={x.dataset}", f"--seed={x.seed}", f"--n_shots={x.n_shots}", f"--decimal={x.decimal}", f"--k={x.k}", f"--method={x.method}", f"--benchmark={x.benchmark}", f"--overwrite={x.overwrite}"]),
                    args
                )
...
def exp4_runshallow_benchmark_0706():
    ...
    exp_name = "exp4_runshallow_benchmark_0708"
    datasets = ["mitbih_arr", "pamap2", "mitbih_auth"]
    # datasets = ["mitbih_arr", "pamap2"]
    n_shots = [1, 2, 3, 4, 8, 16, 32] # [1, 2, 3, 4, 8, 16, 32, 64]
    seeds = range(10)
    ...
    with tqdm(total=len(combinations)) as pbar:
        with ThreadPoolExecutor(max_workers=cpu_count()-2) as executor:
            for seed, model, dataset, n_shot in (combinations):
                ...
                fut = executor.submit(_exp4_runprocess, args)
                fut.add_done_callback(lambda x: pbar.update(1))
                # _exp4_runprocess(args)
...
def exp7_rundeep_benchmark_0708():
    ...
    exp_name = "exp7_rundeep_benchmark_0708"
    datasets = ["mitbih_arr", "pamap2", "mitbih_auth"]
